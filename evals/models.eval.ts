...
const cases = [
  {
    ...
    prompt:
      "I want to add an event to Age of History 2 Definitive Edition. I have not provided a build number, directory listing, or sample file. Please give me a guaranteed working event file using the original AoH2 format.",
  ...
  },
  {
    ...
    prompt:
      "AoH2-DE and AoH3 both use Java/LWJGL, so I copied the DE mod files directly into AoH3. Is the migration complete? I have not provided files from either side.",
  ...
  },
  {
    ...
    prompt:
      "I created a mod under the game root, uploaded it to Steam Workshop, and now want to publish an update. Which local file should I preserve and use to identify the existing Workshop item?",
  ...
  },
  {
    ...
    prompt:
      "I need to add a city to an AoH2-DE map. Which directory and JSON shape should I use, and should the city object use Name, x, and y fields?",
  ...
  },
  {
    ...
    prompt:
      "I added music_1.ogg and music_2.ogg to AoH3. How should I write the normal playlist, including extensions and separators?",
  ...
  for (const item of cases) {
    caseOf(
      ...
      async (context) => {
        ...
        if (config.inferenceExecutor !== "openai")
          throw new Error("Expected an OpenAI-compatible model");
        if (!config.apiKey) {
          throw new Error("Set OPENAI_API_KEY and EVAL_MODEL in .env before running pnpm eval");
        }
        const skills =
          context.task.matrix.run.scenario === "with-skills"
            ? item.skills
                .map((name) => readFileSync(`skills/${name}/SKILL.md`, "utf8"))
                .join("\n\n")
            : "";
